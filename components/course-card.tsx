      ...
      <CardContent className="flex flex-1 flex-col gap-3 pt-0">
        ...
        <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
          <span className="flex items-center gap-1 rounded-md bg-muted/45 px-2 py-1">
            ...
            {course.lessons.length} {cc.lessonsCount}
          </span>
          <span className="flex items-center gap-1 rounded-md bg-muted/45 px-2 py-1">
        ...
        {(showProgress || enrolled) && (
          <div className="space-y-1.5">
            <div className="flex items-center justify-between text-xs">
              <span className="text-muted-foreground">{cc.progressLabel}</span>
        ...
        {enrolled ? (
          <Button className="flex-1 text-sm" size="sm" asChild>
            <Link href={`/courses/${course.id}`} className="flex items-center gap-2">
              ...
              {progressPct > 0 ? cc.continueBtn : cc.startBtn}
            </Link>
          </Button>
        ) : (
          <div className="flex flex-1 gap-2">
            <Button variant="outline" size="sm" className="flex-1 text-sm" asChild>
              <Link href={`/courses/${course.id}`}>{cc.detailsBtn}</Link>
            </Button>
            <Button
              size="sm"
            ...
            >
              {isLoggedIn ? (
                ...
                <Link href="/onboarding">{cc.enrollBtn}</Link>
              )}
            </Button>
          </div>
        )}
