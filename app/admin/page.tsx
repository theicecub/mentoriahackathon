...
export default function AdminPage() {
  ...
  const handleSaveOp = () => {
    ...
    const parsed: Opportunity = {
      ...
      tags: opForm.tags.split(',').map((t) => t.trim()).filter(Boolean),
      link: opForm.link || '#',
    }
  ...
  const handleDeleteOp = (id: string) => {
    if (staticOps.find((o) => o.id === id)) return // can't delete static
  ...
  const stats = [
    { label: 'Всего возможностей', value: allOpportunities.length, icon: Compass, color: 'bg-blue-50 text-blue-600' },
    { label: 'Всего курсов', value: allCourses.length, icon: BookOpen, color: 'bg-amber-50 text-amber-600' },
    { label: 'Курсов добавлено вами', value: adminCourses.length, icon: GraduationCap, color: 'bg-purple-50 text-purple-600' },
    { label: 'Возможностей добавлено', value: adminOpportunities.length, icon: PlusCircle, color: 'bg-green-50 text-green-600' },
  ]
    ...
    <div className="flex min-h-screen flex-col">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Управление курсами и образовательными возможностями Mentoria Hub
              </p>
            </div>
          </div>
        </div>
      ...
      <main className="mx-auto w-full max-w-7xl flex-1 px-4 py-6 sm:px-6">
        ...
        <Tabs value={tab} onValueChange={setTab} className="mb-6">
          <TabsList className="h-auto gap-1 bg-muted p-1">
            {[
              ...
              { value: 'opportunities', label: 'Возможности', icon: Compass },
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Card className="cursor-pointer border-dashed bg-card/80 shadow-none transition-all hover:-translate-y-0.5 hover:border-primary/40" onClick={openAddOp}>
                <CardContent className="flex items-center gap-4 p-5">
                  ...
                  <div>
                    <p className="font-semibold text-foreground">Добавить возможность</p>
                    <p className="text-sm text-muted-foreground">
                      Конкурс, стипендия или программа
                    </p>
                  </div>
                </CardContent>
              </Card>
              <Card className="cursor-pointer border-dashed bg-card/80 shadow-none transition-all hover:-translate-y-0.5 hover:border-primary/40" onClick={openAddCourse}>
                <CardContent className="flex items-center gap-4 p-5">
                  ...
                  <div>
                    <p className="font-semibold text-foreground">Добавить курс</p>
                    <p className="text-sm text-muted-foreground">
                      Новый асинхронный курс Mentoria
                    </p>
                  </div>
                </CardContent>
              </Card>
        ...
        {tab === 'opportunities' && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <p className="text-sm text-muted-foreground">
                Всего: <span className="font-medium text-foreground">{allOpportunities.length}</span> возможностей
                {adminOpportunities.length > 0 && (
                  <span className="ml-2 text-primary">
                    (в т.ч. {adminOpportunities.length} добавлено вами)
                  </span>
                )}
              </p>
              <Button
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border/70 bg-muted/35">
                      ...
                      <th className="hidden px-4 py-3 text-left font-medium text-muted-foreground lg:table-cell">Дедлайн</th>
        ...
        {tab === 'courses' && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <p className="text-sm text-muted-foreground">
                Всего: <span className="font-medium text-foreground">{allCourses.length}</span> курсов
                {adminCourses.length > 0 && (
                  <span className="ml-2 text-primary">
                    (в т.ч. {adminCourses.length} добавлено вами)
                  </span>
                )}
              </p>
              <Button
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border/70 bg-muted/35">
                      ...
                      <th className="hidden px-4 py-3 text-left font-medium text-muted-foreground lg:table-cell">Уроки</th>
                      <th className="px-4 py-3 text-right font-medium text-muted-foreground">Действия</th>
                    </tr>
                  </thead>
                  <tbody>
                    {allCourses.map((c) => {
                      ...
                      const enrolledCount = enrolledCourses.filter((e) => e.courseId === c.id).length
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="hidden px-4 py-3 lg:table-cell">
                            <span className="text-muted-foreground">{c.lessons.length} уроков</span>
                          </td>
                          <td className="px-4 py-3">
                            <div className="flex items-center justify-end gap-1.5">
                              <Button
        ...
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {editingOp ? 'Редактировать возможность' : 'Добавить возможность'}
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-4 pt-2">
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="space-y-1.5">
                <Label htmlFor="op-title">Название *</Label>
                <Input
                  id="op-title"
              ...
              <div className="space-y-1.5">
                <Label htmlFor="op-org">Организация *</Label>
                <Input
                  id="op-org"
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              <div className="space-y-1.5">
                <Label htmlFor="op-cat">Категория</Label>
                <select
                  id="op-cat"
              ...
              <div className="space-y-1.5">
                <Label htmlFor="op-format">Формат</Label>
                <select
                  id="op-format"
                  value={opForm.format}
              ...
              <div className="space-y-1.5">
                <Label htmlFor="op-deadline">Дедлайн</Label>
                <Input
                  id="op-deadline"
                  type="date"
            ...
            <div className="space-y-1.5">
              <Label htmlFor="op-desc">Описание</Label>
              <Textarea
                id="op-desc"
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="space-y-1.5">
                <Label htmlFor="op-req">Требования (каждое с новой строки)</Label>
                <Textarea
                  id="op-req"
                  placeholder={'9–11 класс\nАнглийский B2+\nКоманда 2–5 человек'}
                  rows={3}
              ...
              <div className="space-y-1.5">
                <Label htmlFor="op-grades">Классы (через запятую)</Label>
                <Input
                  id="op-grades"
                  placeholder="9, 10, 11"
                  value={opForm.grades}
                ...
                />
                <Label htmlFor="op-tags" className="mt-2 block">Теги (через запятую)</Label>
                <Input
                  id="op-tags"
                  placeholder="Бизнес, Стартап, Конкурс"
                  value={opForm.tags}
            ...
            <div className="flex justify-end gap-2">
              ...
              >
                ...
                {editingOp ? 'Сохранить изменения' : 'Добавить'}
              </Button>
            </div>
          </div>
        </DialogContent>
      ...
      <Dialog open={courseModalOpen} onOpenChange={setCourseModalOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-xl">
          ...
          <div className="space-y-4 pt-2">
            <div className="space-y-1.5">
              <Label htmlFor="c-title">Название курса *</Label>
              <Input
                id="c-title"
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="c-diff">Уровень</Label>
                <select
                  id="c-diff"
              ...
              <div className="space-y-1.5">
                <Label htmlFor="c-dur">Продолжительность</Label>
                <Input
                  id="c-dur"
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="space-y-1.5">
                <Label htmlFor="c-instructor">Преподаватель *</Label>
                <Input
                  id="c-instructor"
              ...
              <div className="space-y-1.5">
                <Label htmlFor="c-grades">Классы (через запятую)</Label>
                <Input
                  id="c-grades"
                  placeholder="9, 10, 11"
            ...
            <div className="space-y-1.5">
              <Label htmlFor="c-tags">Теги (через запятую)</Label>
              <Input
                id="c-tags"
                placeholder="Python, Алгоритмы, Олимпиада"
